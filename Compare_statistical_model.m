...
for l=1:4
    ...
    road_train_all=repmat(road_train_all,1,3);
    road_fit_all=[road4fit_ARIMA(:,l),road4fit_ets(:,l),road_fit_all];
    road_test_all=repmat(road_test_all,1,3);
    road_pre_all=[road4pre_ARIMA(:,l),road4pre_ets(:,l),road_pre_all];
    % compute mean absolute error
    mae_fit=mean(abs(road_fit_all-road_train_all),1,'omitnan'); 
    mae_pre=mean(abs(road_pre_all-road_test_all),1,'omitnan');
