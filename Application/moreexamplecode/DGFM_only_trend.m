function [ x_fit,par ] = DGFM_only_trend( x,omega,order,f )
% DGFM function
% input:
% x: time series data
...
par=pinv(H'*H)*H'*X;
% x_hat_fit=[x(1);H*par];
x_hat_fit=[x(1);H(:,1:3)*par(1:3)]; % y of H is periodic
alpha=par(1);
c=par(2);
theta=par(3:end);
%% time sponse
...
for l=1:f
    ...
    % x_hat_fore(l)=alpha*y_m+c*(m+l)+[1,cossinl]*theta;
    x_hat_fore(l)=alpha*y_m+c*(m+l)+theta(1);
