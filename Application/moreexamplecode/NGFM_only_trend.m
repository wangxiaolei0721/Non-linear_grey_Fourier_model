function [ x_fit_pre,par ] = NGFM_only_trend( x,omega,order,gamma,sigma,test )
...
%% time sponse
% x_fit=Phi'*alpha+A_0+costimes*A1AN+sintimes*B1BN; % Phi is a symmetric matrix
x_fit=Phi'*alpha+A_0; 
x_fit=[x(1);x_fit];
y_end=xcum(length(x));
Phit=zeros(1,m);
