...
crudeoil_ets <- ets(crudeoil_train_val_ts)
ets_order <- crudeoil_ets$components
...
while ((k+train_data_length+test_step-1) <= NROW(train_test_data_index)) {
  ...
  crudeoil_train_ets <- ets(crudeoil_train_ts, model = model0, as.logical(damped0))
  crudeoil_test_fore <- forecast(crudeoil_train_ets, h = test_step)
  # train value
  crudeoil_train_fit <-as.vector(crudeoil_train_ets$fitted)
